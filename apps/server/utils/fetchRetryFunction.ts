...
const fetchLogger = logger.child({ fileName: 'utils/fetchRetryFunction.ts' });
...
export async function fetchWithRetry<T>(url: string, retries = 3, initialRetries = 3): Promise<T | null> {
    ...
    try {
      ...
      if (error.response?.status === 429 && retries > 0) {
        ...
        fetchLogger.warn(`Rate limited for ${url}. Retrying in ${Math.round(backoffDelay)}ms (attempt ${attemptNumber}/${initialRetries})...`);
        await new Promise(resolve => setTimeout(resolve, backoffDelay));
        return fetchWithRetry<T>(url, retries - 1, initialRetries);
      }
  
      fetchLogger.error(`Failed to fetch ${url}: ${error.message}`);
      if (retries > 0) {
        const attemptNumber = initialRetries - retries + 1;
        ...
        fetchLogger.warn(`Retrying ${url} in ${Math.round(delay)}ms (attempt ${attemptNumber}/${initialRetries})...`);
